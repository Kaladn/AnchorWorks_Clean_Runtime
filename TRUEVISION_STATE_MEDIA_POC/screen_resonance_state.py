...
class ScreenResonanceState:
    ...
    def __init__(self,
        ...
        Args:
            grid_size: Grid dimensions. Int keeps legacy square mode, tuple uses (rows, cols).
        ...
        if isinstance(grid_size, tuple):
            ...
            self.grid_cols = int(grid_size[1])
        else:
            self.grid_rows = int(grid_size)
            self.grid_cols = int(grid_size)
        self.grid_size = self.grid_rows if self.grid_rows == self.grid_cols else (self.grid_rows, self.grid_cols)
    ...
    def _setup_spatial_masks(self):
        ...
        # Center mask: legacy 10x10 keeps central 4x4; rectangular grids scale per axis.
        center_rows = max(1, min(rows, round(rows * 0.4)))
        center_cols = max(1, min(cols, round(cols * 0.4)))
        center_row_start = (rows - center_rows) // 2
        center_col_start = (cols - center_cols) // 2
        self.center_mask = np.zeros((rows, cols), dtype=bool)
        self.center_mask[
            center_row_start:center_row_start + center_rows,
            center_col_start:center_col_start + center_cols,
        ] = True

        # Edge mask (border cells)
        self.edge_mask = np.zeros((rows, cols), dtype=bool)
        ...
        mid = rows // 2
        self.upper_mask = np.zeros((rows, cols), dtype=bool)
        self.upper_mask[:mid, :] = True
        self.lower_mask = np.zeros((rows, cols), dtype=bool)
    ...
    def update(self, grid_t: np.ndarray) -> Dict[str, float]:
        ...
        """
        if grid_t.shape != (self.grid_rows, self.grid_cols):
            raise ValueError(
                f"grid shape {grid_t.shape} does not match resonance shape "
